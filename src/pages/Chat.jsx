...
//   // 1. Force scroll to bottom instantly ONLY when switching chats or views
//   useEffect(() => {
//     endRef.current?.scrollIntoView({ behavior: 'auto' });
//   }, [selectedTeamId, selectedDmUser, viewMode]);

//   // 2. Smart scroll: Only scroll for new messages IF the user is already at the bottom
//   useEffect(() => {
...
//       gap: '14px',
//       height: 'calc(100vh - 80px)' // Slightly reduced to prevent scrollbars on laptops
//     },
//     card: {
...
//       minHeight: 0             // NEW: Crucial for preventing mobile overflow
//     },
//     sideHeader: {
//       padding: '14px',
...
//     },
//     sideScroll: {
//       flex: 1,                 // NEW: Replaces the hardcoded calc() height
//       minHeight: 0,            // NEW
//       overflowY: 'auto',
//       padding: '10px'
//     },
...
//       top: '66px',
//       left: '12px',
//       width: 'calc(100vw - 40px)', // <-- Makes it responsive on phones
//       maxWidth: '290px',          // <-- Keeps it normal size on laptops
//       maxHeight: '360px',
//       overflowY: 'auto',
//       backgroundColor: colors.glassStrong,
//       border: `1px solid ${colors.border}`,
...
//             grid-template-rows: 25vh 1fr !important; /* 25% screen for list, rest for chat */
//             height: calc(100dvh - 100px) !important; /* dvh fixes mobile address bar bounce */
//             gap: 12px !important;
//           }
//           .side-panel {
...
//           .chat-frame {
//              grid-template-rows: 20vh 1fr !important; /* Even smaller list on phones */
...
export default function Chat() {
  ...
  const styles = {
    // shell: {
    //   maxWidth: '1240px',
    //   margin: '0 auto', /* CHANGED: Removed top/bottom margin to prevent overflow */
    //   padding: '14px',  /* Changed to apply padding around the whole frame */
    //   fontFamily: '"Segoe UI", "SF Pro Text", Tahoma, sans-serif',
    //   color: colors.textMain,
    //   height: '100dvh', /* CHANGED: Added explicit height to the shell */
    ...
    //   gridTemplateColumns: '300px 1fr',
    //   gap: '14px',
    //   flex: 1, /* CHANGED: Replaced calc() height with flex to perfectly fill remaining space */
    //   minHeight: 0 /* Crucial for flex children to not overflow */
    // },
    ...
    sideItemMain: {
      minWidth: 0,
      overflow: 'hidden' // Prevents team name from pushing the card width
    },
    avatar: (name, imgUrl) => ({
      width: '40px',
      height: '40px',
    ...
    chatCard: {
      ...
      border: `1px solid ${colors.border}`,
      overflow: 'hidden',
      display: 'flex', /* CHANGED from grid to flex to fix bottom blank space */
      flexDirection: 'column',
      minHeight: 0             
    },
    chatTop: {
    ...
    thread: {
      flex: 1, /* CHANGED: Tells thread to take up exactly the remaining vertical space */
      minHeight: 0,
      overflowY: 'auto',
      padding: '18px 22px',
      background: `radial-gradient(circle at top right, rgba(79, 140, 255, 0.14) 0%, ${colors.glassSoft} 48%, ${colors.glass} 100%)`
    ...
    bubble: (mine) => ({
      ...
      overflowWrap: 'anywhere',   // Forces long strings (like 'aaaaaaa...') to wrap
      whiteSpace: 'pre-wrap',     // Preserves spaces/newlines but wraps normally
    }),
    msgName: (mine) => ({
      fontSize: '11px',
      marginBottom: '3px',
    ...
    inputWrap: {
      ...
      alignItems: 'center',
      gap: '10px',
      flexShrink: 0 /* CHANGED: Ensures the input box never shrinks */
    }
  };
  ...
  return (
    // <div className="chat-shell" style={styles.shell}>
    //   <style>{`
    //     /* Overrides for Mobile/Tablet */
    //     @media (max-width: 980px) {
    //       .chat-shell {
    ...
    //       .chat-frame {
    //         grid-template-columns: 1fr !important;
    //         grid-template-rows: 20vh 1fr !important; /* 20% screen for list, rest for chat */
    //         gap: 10px !important;
    //       }
    //       .side-panel {
    //         height: 100% !important;
    ...
    //   `}</style>
      ...
      <style>{`
        ...
        @media (max-width: 980px) {
          .chat-shell {
             padding: 8px !important; /* Smaller padding for more space */
          }
          .chat-frame {
            grid-template-columns: 1fr !important;
            /* 30vh for list, rest for chat to give more room for messages */
        ...
        @media (max-width: 600px) {
            .chat-frame {
                grid-template-rows: 25vh 1fr !important;
            }
        }
      `}</style>
      {error && <p style={{ color: colors.red, flexShrink: 0 }}>{error}</p>}
