...
const formatTime = (value) => {
    ...
    // Check if it's an ISO string (contains a 'T' separating date and time)
    if (typeof value === 'string' && value.includes('T')) {
      const timePart = value.split('T')[1];
      
      // ONLY check the time portion for timezone offsets (+ or -)
      if (!value.endsWith('Z') && !timePart.includes('+') && !timePart.includes('-')) {
        safeValue += 'Z'; 
      }
    }
